...
public class FirebaseDatabaseService implements FillingsService, ProductService, OrderService {
    ...
    @Override
    public LiveData<Map<String, List<Filling>>> getFillingsByProduct(Bakery product, final String type) {
        return new DatabaseReferenceLiveData<>(
                rootReference.child("fillings").child(product.fillingId()), liveErrors,
                new DataSnapshotConverter<Map<String, List<Filling>>>() {
